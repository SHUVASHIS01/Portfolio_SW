    ...
    >
      <div className="p-6 flex flex-col justify-between flex-1">
        <div className="flex flex-col gap-4">
          ...
          {/* Description */}
          <p className="text-xs sm:text-sm leading-relaxed text-white line-clamp-2">
            {project.description}
          </p>
          ...
          <div className="flex flex-wrap gap-1.5 mt-1">
            {visibleStack.map((t) => (
              <span key={t} className="chip text-[10px] px-2 py-0.5 bg-black/20 border-white/10 text-white">
                {t}
              </span>
            ))}
            {remainingCount > 0 && (
              <span className="chip text-[10px] px-2 py-0.5 text-white bg-transparent border-transparent">
                +{remainingCount} more
              </span>
            )}
          </div>
